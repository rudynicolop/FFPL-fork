...
Proof.
  ...
  - intros (x & e & -> & Hsem1 & Hsem2). simpl.
    apply andb_prop_intro. split.
    * by apply IHA1 in Hsem1.
    * by apply IHA2 in Hsem2.
Qed.
...
(** These are the new compatibility lemmas *)
Lemma compat_pair Gamma e1 e2 A B :
  Gamma |= e1 : A ->
  Gamma |= e2 : B ->
...
Proof.
  ...
  intros gamma Hctx. simpl.
  simp type_interp.
  specialize (He _ Hctx).
  simp type_interp in He.
  destruct He as (v & Hstep & Hsem).
  simp type_interp in Hsem.
  destruct Hsem as (v1 & v2 & -> & Hv1 & Hv2).
  eexists.
  split.
  - econstructor. done.
  - done.
